...
import { useAssignConductor } from "../hooks/useConductors";
import type { Conductor } from "../constant/conductor.types";

export function ConductorAssignPanel({ conductor }: { conductor: Conductor }) {
  const assign = useAssignConductor(conductor._id);
  ...
  const [vehicleId, setVehicleId] = useState(
    conductor.assigned.vehicleId ?? ""
  );
  const [routeId, setRouteId] = useState(conductor.assigned.routeId ?? "");
  const [scheduleId, setScheduleId] = useState(
    conductor.assigned.scheduleId ?? ""
  );
